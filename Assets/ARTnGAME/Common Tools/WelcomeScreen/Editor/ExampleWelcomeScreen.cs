...
changes in object materials. The system also supports screen space rain, besides rain running on objects and rain drop ripples.",
        ...
        private static readonly ScrollViewGuiSection MainContentSection = new ScrollViewGuiSection(
            "", (screen) =>
            {
                ...
                //GuiSection MidSection = new GuiSection(
                using (LayoutHelper.LabelWidth(200))
                {
                    // "Massive Offer:Envirnonment Building Bundle for $69 for a limited time";
                    ...
                    GUILayout.Label(
                              @"Big Offer:
Upgrade to Environment Building Bundle for $79 for a limited time
from Toon Effects Maker URP until 28th Dec 23 ($460 full price)!",
                              headStyle//screen.TextStyle, headStyle
                          );
                    //if (Event.current.type == EventType.MouseUp && yourLabelRect.Contains(Event.current.mousePosition))
                    //{
                    //    Application.OpenURL("https://assetstore.unity.com/packages/vfx/shaders/environment-building-bundle-238238");
                    //}
                    //GUILayout.Label(@"Envirnonment Building Bundle",
                    //          headStyle);                    
                    ProductPreferenceBase.RenderURL("Envirnonment Building Bundle Asset Store", $"https://assetstore.unity.com/packages/vfx/shaders/environment-building-bundle-238238");
                    // };
                }
                // );
                //GUI.skin.label.fontSize = 12;
                const int sectionBreakHeight = 19;
                GUILayout.Space(sectionBreakHeight);
